...
main = tkinter.Tk()
main.title("AI-DRIVEN DETECTION OF INJECTION ATTACKS IN API’S USING BIDIRECTIONAL RECURRENT NEURAL NETWORKS")
...
def runVanilla():
    ...
    vanilla_model.add(SimpleRNN(100, input_shape=(X_train.shape[1], X_train.shape[2])))
    ...
    if os.path.exists("model/vanilla_weights.hdf5") == False:
        ...
        vanilla_model.load_weights("model/vanilla_weights.hdf5")
...
def runGRU():
    ...
    gru_model.add(GRU(100, input_shape=(X_train.shape[1], X_train.shape[2])))
    ...
    if os.path.exists("model/gru_weights.hdf5") == False:
        ...
        gru_model.load_weights("model/gru_weights.hdf5")
    #perform prediction on test data   
...
font = ('times', 16, 'bold')
title = Label(main, text='AI-DRIVEN DETECTION OF INJECTION ATTACKS IN API’S USING BIDIRECTIONAL RECURRENT NEURAL NETWORKS')
